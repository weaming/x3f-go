...
// CollectBadPixels 收集所有坏点位置
func CollectBadPixels(file *x3f.File, imageWidth, imageHeight uint32, colors int) []BadPixel {
	...
	if cameraID, ok := file.GetCAMFUint32("CAMERAID"); ok {
		...
		if cameraID == 0x0F { // X3F_CAMERAID_SDQ
			if colors == 1 {
		...
		} else if cameraID == 0x10 { // X3F_CAMERAID_SDQH
			if colors == 1 {
